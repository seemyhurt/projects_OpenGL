...
GLdouble lastX, lastY;		//Íà÷àëüíîå ïîëîæåíèå êàìåðû
GLfloat deltaTime = 0.0f;	// Âðåìÿ, ïðîøåäøåå ìåæäó ïîñëåäíèì è òåêóùèì êàäðîì
GLfloat lastFrame = 0.0f;  	// Âðåìÿ âûâîäà ïîñëåäíåãî êàäðà
Camera camera(glm::vec3(0.0f, 0.0f, 3.0f));
bool keys[1024];					//Ìàññèâ ïåðåìåííûõ ëîãè÷åñêîãî òèïà, ïðåäñòàâëÿþùèé íàæàòûå èëè îòïóùåííûå ñîñòîÿíèÿ êëàâèø
...
int main()
{
    ...
    glfwInit();
    //Íàñòðîéêà GLFW
    //Çàäàåòñÿ ìèíèìàëüíàÿ òðåáóåìàÿ âåðñèÿ OpenGL. 
    //Ìàæîðíàÿ 
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
    //Ìèíîðíàÿ
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
    //Óñòàíîâêà ïðîôàéëà äëÿ êîòîðîãî ñîçäàåòñÿ êîíòåêñò
    glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
    //Âûêëþ÷åíèå âîçìîæíîñòè èçìåíåíèÿ ðàçìåðà îêíà
...
GLuint create_texture(const char* filename)
{
    ...
    // Óñòàíàâëèâàåì íàñòðîéêè ôèëüòðàöèè è ïðåîáðàçîâàíèé
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    //ñãåíåðèðîâàòü äàííûå òåêñòóðû èñïîëüçóÿ ïðåäâàðèòåëüíî çàãðóæåííîå èçîáðàæåíèå
    int width, height;
    unsigned char* image2 = stbi_load(filename, &width, &height, 0, SOIL_LOAD_RGB);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image2);
    glGenerateMipmap(GL_TEXTURE_2D);
    //îñâîáîæäåíèå ó÷àñòêà ïàìÿòè, âûäåëåííîãî ïîä çàãðóæåííîå èçîáðàæåíèå, è îòâÿçêà îáúåêòà òåêñòóðû
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    // Êîãäà ïîëüçîâàòåëü íàæèìàåò ESC, ìû óñòàíàâëèâàåì ñâîéñòâî WindowShouldClose â true, 
    // è ïðèëîæåíèå ïîñëå ýòîãî çàêðîåòñÿ
...
{
    // Camera controls
    //Âñÿêèé ðàç, êîãäà ìû íàæèìàåì îäíó èç êëàâèø WASD, ïîëîæåíèå êàìåðû îáíîâëÿåòñÿ â ñîîòâåòñòâèè ñ å¸ íîâûìè êîîðäèíàòàìè. 
    if (keys[GLFW_KEY_W])
        camera.ProcessKeyboard(FORWARD, deltaTime);
    if (keys[GLFW_KEY_S])
        camera.ProcessKeyboard(BACKWARD, deltaTime);
