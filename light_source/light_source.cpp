...
int main()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	//сгенерировать данные текстуры используя предварительно загруженное изображение
    std::string containeSpecularPath = std::string("images/container_specular.png");
    unsigned char* image2 = SOIL_load_image(containeSpecularPath.c_str(), &width, &height, 0, SOIL_LOAD_RGB);
